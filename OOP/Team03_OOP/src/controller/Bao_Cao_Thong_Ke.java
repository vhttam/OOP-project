...
public class Bao_Cao_Thong_Ke {
	...
	public void locNhanVienTheoChucVu( List<NhanVien> dsNhanVien) {
		ArrayList <NhanVien> dsBanHang = new ArrayList<>();
		ArrayList <NhanVien> dsThuNgan = new ArrayList<>();
		ArrayList <NhanVien> dsQuanLiKho = new ArrayList<>();
		if (dsNhanVien == null || dsNhanVien.isEmpty()) {
			System.out.println("Danh sach nhan vien rong, khong co gi de loc.");
			return;
		}
		for (NhanVien nv : dsNhanVien){
			if (nv.getchucvu().equalsIgnoreCase("Ban Hang")){
				dsBanHang.add(nv);
			}
		}
		for (NhanVien nv : dsNhanVien){
			if (nv.getchucvu().equalsIgnoreCase("Thu Ngan")){
				dsThuNgan.add(nv);
			}
		}
		for (NhanVien nv : dsNhanVien){
			if (nv.getchucvu().equalsIgnoreCase("Quan Li Kho")){
				dsQuanLiKho.add(nv);
			}
		}

		if (dsBanHang.isEmpty()) {
        System.out.println("Khong tim thay nhan vien 'Ban Hang' nao.");
    } else {
		System.out.println("\n--- DANH SACH NHAN VIEN BAN HANG ---");
		for (NhanVien nv : dsBanHang) {
			nv.xuat();
			System.out.println("-----------------------------------------");
		}
	}
		if (dsThuNgan.isEmpty()) {
		System.out.println("Khong tim thay nhan vien 'Thu Ngan' nao.");
	} else {
		System.out.println("\n--- DANH SACH NHAN VIEN THU NGAN ---");
		for (NhanVien nv : dsThuNgan) {
			nv.xuat();
			System.out.println("-----------------------------------------");
		}
	}
		if (dsQuanLiKho.isEmpty()) {
		System.out.println("Khong tim thay nhan vien 'Quan Li Kho' nao.");
	} else {
		System.out.println("\n--- DANH SACH NHAN VIEN QUAN LI KHO ---");
		for (NhanVien nv : dsQuanLiKho) {
			nv.xuat();
			System.out.println("-----------------------------------------");
		}
	}
